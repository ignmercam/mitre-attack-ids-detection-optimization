...
def get_summary_metrics(summary_path):
    ...
    for dict_key, txt_header in blocks_map.items():
        if txt_header in content:
            ...
            m = {}
            try:
                # FIX 1: Soporta tanto "Active Rules" como "Active IDs"
                m['active_rules'] = int(re.search(r'Active (?:Rules|IDs):\s*(\d+)', blk).group(1))

                # FIX 2: Soporta tanto "Removed SIDs" como "Removed IDs"
                match_rem = re.search(r'Removed (?:SIDs|IDs):\s*(.*)', blk)
                if match_rem:
                ...
                # Opcional: Imprime el error por terminal para que no vuelva a pasar desapercibido
                print(f"      [!] Error parseando métricas en {dict_key}: {e}")
                pass
    return metrics
...
def main():
    ...
    os.chdir(base_dir)
    print("=== TFG: GENERATOR OF FINAL TABLES & GLOBAL SUMMARY (FG-PA) ===\n")

    master_dir = "Resultados_FG-PA_GM"

    plantilla_path = os.path.join(master_dir, "plantilla.xlsx")
    if not os.path.exists(plantilla_path):
        plantilla_path = "plantilla.xlsx"

    salida_tablas = os.path.join(master_dir, "FG-PA_GM_Final_Tables.xlsx")
    salida_resumen = os.path.join(master_dir, "Global_G-Mean_FG-PA_Summary.xlsx")

    if not os.path.exists(plantilla_path):
        print(f"[!] Error: Template '{plantilla_path}' no encontrada.")
        return

    print("[*] TAREA 1: Rellenando plantilla detallada por hoja de severidad...")
    ...
    # Mapeo para el bloque principal (columnas unificadas)
    column_mapping = {
        ('FG', 'IPS'): 2,
        ('FG', 'APP'): 3,
        ('PA', 'IPS'): 4,
    ...
    # Mapeo preciso para el historial iterativo inferior (columnas divididas de la plantilla)
    history_col_mapping = {
        ('FG', 'IPS'): (3, 4),  # Columnas C y D
        ('FG', 'APP'): (6, 7),  # Columnas F y G
        ('PA', 'IPS'): (9, 10),  # Columnas I y J
        ('PA', 'APP'): (12, 13)  # Columnas L y M
    }
    ...
    for s in escenarios:
        ...
        ws_new.title = f"Sev <= {s}"
        print(f"\n  >> Rellenando Hoja: Sev <= {s}")

        # Localizar bloques principales
        row_bounds = {}
        ...
        # Localizar el inicio de la tabla del historial inferior
        row_hist_header = None
        for r in range(1, ws_new.max_row + 20):
            for c in range(1, 15):
                if str(ws_new.cell(row=r, column=c).value).strip() == "Nº SIDs eliminados":
                    row_hist_header = r
                    break
            if row_hist_header:
                break
        ...
        for engine in ['FG', 'PA']:
            for track in ['IPS', 'APP']:
                col_num = column_mapping.get((engine, track))
                res_folder = os.path.join(master_dir, engine, track, f"Sev_{s}")
                ...
                # --- RECOLECTAR DATOS PARA EL RESUMEN GLOBAL Y EL HISTORIAL ---
                ...
                if m_cons and m_agr:
                    ...
                    label_global = f"{engine}_{track} [Sev>={s}]"
                    ...
                    global_summary_data.append({
                        ...
                        'Track': track,
                        'Scenario': f"Sev >= {s}",
                        'Initial_Active_Rules': m_cons['active_rules'],
                        ...
                        'Rules_Removed_Optimization': rules_removed,
                        'Final_Active_Rules': m_agr['active_rules'],
                ...
                # --- 1. VOLCAR LAS MÉTRICAS EN LA TABLA PRINCIPAL SUPERIOR ---
                for b_name in blocks_in_txt:
                    ...
                    write_metric(ws_new, "#SIDs/AttackID", b_start, b_start + 15, col_num, m['removed_count'],
                                 align_left)
                    write_metric(ws_new, "#Alertas total", b_start, b_start + 15, col_num, m['a_tot'], align_left)
                    write_metric(ws_new, "%Detección_pcaps", b_start, b_start + 15, col_num, f"{m['tpr']:.2f}%",
                                 align_left)
                    write_metric(ws_new, "#Alertas_TP", b_start, b_start + 15, col_num, m['a_tp'], align_left)
                    write_metric(ws_new, "#Alertas TP (Diff", b_start, b_start + 15, col_num, m['a_tp_diff'],
                                 align_left)
                    write_metric(ws_new, "#Alertas_FP", b_start, b_start + 15, col_num, m['a_fp'], align_left)
                    write_metric(ws_new, "#Alertas FP (Diff", b_start, b_start + 15, col_num, m['a_fp_diff'],
                                 align_left)
                    write_metric(ws_new, "Usabilidad", b_start, b_start + 15, col_num, m['usab'], align_left)

                # --- 2. VOLCAR EL HISTORIAL PASO A PASO EN LA TABLA INFERIOR ---
                if row_hist_header and history_f2:
                    step_col, gm_col = history_col_mapping.get((engine, track))
                    current_row = row_hist_header + 1

                    for step_num, gm_val in history_f2:
                        c1 = ws_new.cell(row=current_row, column=step_col, value=step_num)
                        c1.border, c1.alignment = border_thin, align_center

                        c2 = ws_new.cell(row=current_row, column=gm_col, value=gm_val)
                        c2.border, c2.alignment = border_thin, align_center

                        current_row += 1

    # Eliminamos la hoja base vacía y guardamos
    wb_tables.remove(ws_base)
    wb_tables.save(salida_tablas)
    print(f"\n  -> ✅ Guardado fichero detallado (Con historiales incrustados): {salida_tablas}")
    ...
    if global_summary_data:
        ...
        df_summary.sort_values(by=['Engine', 'Track', 'Scenario'], inplace=True)
        ...
        sorted_labels = sorted(history_by_scenario.keys())
